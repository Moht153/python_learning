...
from blogtoken.views import make_token, decode_token
...
def users(request, username=None):
	...
	if request.method == 'GET':
		...
		if username:
			...
			if request.GET.keys():
				...
				result = {'code': 200, 'username': username, 'data': {
					'info': user.info, 'sign': user.sign, 'avatar': str(user.avatar), 'nickname': user.nickname
				}}
				return JsonResponse(result)
		else:
			# /v1/users
		...
		try:
			auser = UserProfile.objects.create(username=user_name)
			auser.email = email
			auser.password = passwd1
			auser.sign = sign
			auser.info = info
			auser.save()
...
def user_avatar(request, username):
	...
	if request.method != "POST":
		result = {'code': 213, 'error': 'I need POST'}
		return JsonResponse(result)
	...
	# TODO 判断url中的username 是否跟token中的username一致，若不一致，则返回
	# 跟进： 已写到装饰器中
